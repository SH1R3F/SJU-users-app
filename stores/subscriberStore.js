...
export const useSubscriberStore = defineStore("subscriberStore", {
	...
	actions: {
		async fetchEvents() {
			...
			const { data, error, refresh } = await useMyFetch("/subscribers/events", {
		...
		updateSubscriber: async (body) => {
			...
			const { data, error } = await useMyFetch("/subscribers/profile", {
		...
		updatePassword: async (body) => {
			...
			const { data, error } = await useMyFetch("/subscribers/profile/password", {
				key: "update-password",
				method: "POST",
				body,
			})
